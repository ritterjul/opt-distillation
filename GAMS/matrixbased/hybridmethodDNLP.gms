$ontext
topology as variable, but relaxed integrality
discontinuous formulation of constraints (max)
$offtext
...
x.up(i,j) = 1;
Positive variable y(i,j,k,l);
y.up(i,j,k,l) = 1;
Positive variable z(i,j,k,l);
z.up(i,j,k,l) = 1;
...
max(LK(i,j)-ord(c)+1,0)*(sum((k,l)$dE(i,j,k,l),B(i,j,i,l,c))) =e= 0;
...
max(ord(c)-HK(i,j)+1,0)*(sum(l$hE(i,j,i,l),D(i,j,i,l,c))) =e= 0;
...
* =e= sum(m,sum(l$hE(i,l,i,j),Di(i,l,i,j,m)))

Equation SecondUnderwood(i,j,c);
SecondUnderwood(i,j,c)$(U(i,j) and ord(j) <= NC-1 and ord(c) >= ord(i) and (ord(c) <= NC + ord(i) - ord(j) -1)) ..
x(i,j)*max(ord(c)-LK(i,j)+1,HK(i,j)-ord(c),0)*(V(i,j)-sum(m,alpha(m)*sum(l$hE(i,j,i,l),D(i,j,i,l,m))/(alpha(m)-theta(i,j,c)))) =g= 0;
...
*if product is produced by two columns, they must actually be one column -> vapor flows are equal
...
Model hybridmethodDNLP /all/;

Option DNLP = KNITRO;

*Option DNLP = kestrel;
*hybridmethodDNPL.OptFile = 1;

Solve hybridmethodDNLP using DNLP minimizing Reboiler;
